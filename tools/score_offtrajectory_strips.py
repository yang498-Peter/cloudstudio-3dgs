...
# name=dir pairs on the command line replace the historical arm table, so a
# re-scored strip set can be compared without editing the file.
if len(sys.argv) > 1:
    arms = {a.split("=", 1)[0]: Path(a.split("=", 1)[1]) for a in sys.argv[1:]}
