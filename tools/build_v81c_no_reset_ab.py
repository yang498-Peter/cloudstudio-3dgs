"""Build V81c: isolate step-3000 behavior with reset deferred to step 6000."""
...
TARGET = BASE / "v81c_no_reset_ab" / "tile0_pre3100"
...
def main() -> int:
    ...
    if int(payload.get("step", -1)) != 2999:
        raise ValueError("V81c requires the exact V81a step-2999 checkpoint")

    config.update(
        {
            "run_id": "snow-tile0-v81c-no-reset-ab-pre3100",
    ...
    gate.setdefault("evidence", {})["v81c_no_reset_ab"] = {
        ...
        "purpose": "same-random-state control that excludes only the step-3000 opacity reset",
    }
    gate["adaptive_growth"] = dict(gate["adaptive_growth"])
    gate["adaptive_growth"].update(
        {
            "profile": "v81c_no_reset_ab_pre3100",
    ...
    print("V81c: step-3000 lifecycle without opacity reset, settle through step 3099")
    return 0
