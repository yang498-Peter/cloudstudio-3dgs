"""Build V78b: one more growth event followed by a full settle window."""
...
SOURCE = BASE / "v78a_post_split_settle" / "tile0_pre1300"
TARGET = BASE / "v78b_second_detail_cycle" / "tile0_pre1400"
...
def main() -> int:
    ...
    if int(payload.get("step", -1)) != 1299:
        raise ValueError("V78b requires the exact V78a step-1299 checkpoint")

    config.update(
        {
            "run_id": "snow-tile0-v78b-second-detail-cycle-pre1400",
            "output_dir": str(TARGET / "training_ewa"),
            "mipmap_pipeline_gate": str(TARGET / "training_gate.json"),
            "resume_checkpoint": str(checkpoint),
            "controlled_stop_after_steps": 1399,
            "checkpoint_every": 1399,
            "checkpoint_keep_every": 1399,
        }
    )
    ...
    gate.update(
        {
            ...
            "next_required_stage": "second_growth_settle_evaluation",
        }
    )
    ...
    gate.setdefault("evidence", {})["v78b_second_detail_cycle"] = {
        "source_checkpoint_sha256": _sha256_file(checkpoint),
        "source_completed_steps": 1299,
        "controlled_stop_completed_steps": 1399,
        "parameter_changes": {},
        "purpose": (
            "execute the step-1300 detail lifecycle and give its newborns a "
            "complete settle window before the step-1400 event"
        ),
    }
    gate["adaptive_growth"] = dict(gate["adaptive_growth"])
    gate["adaptive_growth"].update(
        {
            "profile": "v78b_second_detail_cycle_pre1400",
    ...
    print("V78b: event=1300 settle stop=1399 before next lifecycle")
    return 0
