"""Build V82b: settle mesh depth through step 3199 before topology step 3200."""
...
SOURCE = BASE / "v82a_mesh_depth_first_on_boundary" / "tile0_step3139"
TARGET = BASE / "v82b_mesh_depth_settle" / "tile0_pre3200"
...
def main() -> int:
    ...
    if int(payload.get("step", -1)) != 3139:
        raise ValueError("V82b requires the exact V82a step-3139 checkpoint")

    config.update(
        {
            "run_id": "snow-tile0-v82b-mesh-depth-settle-pre3200",
            "output_dir": str(TARGET / "training_ewa"),
            "mipmap_pipeline_gate": str(TARGET / "training_gate.json"),
            "resume_checkpoint": str(checkpoint),
            "controlled_stop_after_steps": 3199,
            "checkpoint_every": 3199,
            "checkpoint_keep_every": 3199,
        }
    )
    ...
    gate.update(
        {
            ...
            "next_required_stage": "mesh_depth_settle_joint_evaluation",
        }
    )
    ...
    gate.setdefault("evidence", {})["v82b_mesh_depth_settle"] = {
        "source_checkpoint_sha256": _sha256_file(checkpoint),
        "source_completed_steps": 3139,
        "controlled_stop_completed_steps": 3199,
        "next_topology_event_step": 3200,
        "purpose": "isolate mesh-depth settling from the next clone/split/cull event",
    }
    gate["adaptive_growth"] = dict(gate["adaptive_growth"])
    gate["adaptive_growth"].update(
        {
            "profile": "v82b_mesh_depth_settle_pre3200",
    ...
    print("V82b: mesh-depth-on settle to step 3199, before topology step 3200")
    return 0
