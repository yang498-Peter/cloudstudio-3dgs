"""Build V78a: settle V77b step-1200 Split children before another event."""
...
SOURCE = BASE / "v77b_visible_opacity" / "tile0_boundary1202"
TARGET = BASE / "v78a_post_split_settle" / "tile0_pre1300"
...
def main() -> int:
    ...
    if int(payload.get("step", -1)) != 1202:
        raise ValueError("V78a requires the exact V77b step-1202 checkpoint")

    config.update(
        {
            "run_id": "snow-tile0-v78a-post-split-settle-pre1300",
            "output_dir": str(TARGET / "training_ewa"),
            "mipmap_pipeline_gate": str(TARGET / "training_gate.json"),
            "resume_checkpoint": str(checkpoint),
            "controlled_stop_after_steps": 1299,
            "checkpoint_every": 1299,
            "checkpoint_keep_every": 1299,
        }
    )
    ...
    gate.update(
        {
            ...
            "next_required_stage": "post_split_settle_evaluation",
        }
    )
    ...
    gate.setdefault("evidence", {})["v78a_post_split_settle"] = {
        "source_checkpoint_sha256": _sha256_file(checkpoint),
        "source_completed_steps": 1202,
        "controlled_stop_completed_steps": 1299,
        "parameter_changes": {},
        "purpose": (
            "allow step-1200 Split children 97 optimization steps to settle; "
            "stop immediately before the next step-1300 lifecycle event"
        ),
    }
    gate["adaptive_growth"] = dict(gate["adaptive_growth"])
    gate["adaptive_growth"].update(
        {
            "profile": "v78a_post_split_settle_pre1300",
    ...
    print("V78a: resume=1202 settle-only stop=1299 before next lifecycle")
    return 0
