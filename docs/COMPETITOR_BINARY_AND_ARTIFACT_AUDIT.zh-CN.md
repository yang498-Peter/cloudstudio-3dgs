...
### 5.1 裁剪阈值缩放的独立复核（2026-08-29 夜）

首轮报告称"字段 `[0x124/0x128/0x130]` 在纯裁剪步上分别 ×0.25 / ×5.0 / ×5.0，阈值 A 在训练前半段 ×2"。该结论只有一次来源、未经复核，因此**没有**据此修改训练器。随后用 capstone 独立反汇编 `0xcf26f–0xcf300`，逐条指令核对，结果与首轮描述有三处实质出入。

实际指令序列（已验证）：

```
0x0cf26f  movss    xmm1, [r14 + 0x128]        ; T_B
0x0cf27b  movss    xmm2, [r14 + 0x124]        ; T_A
0x0cf287  movss    xmm3, [r14 + 0x130]        ; T_C
0x0cf293  mov      eax,  [r14 + 0xe0]         ; 总迭代数
0x0cf29a  cdq / sub eax,edx / sar eax,1       ; eax = 总迭代数 / 2（有符号）
0x0cf29f  cmp      r12d, eax
0x0cf2a2  jge      0xcf2ae                    ; 当前步 >= 一半 -> 跳过加倍
0x0cf2a4  movaps   xmm0, xmm1
0x0cf2a7  addss    xmm0, xmm1                 ; T_B + T_B（即 ×2）
0x0cf2ab  cvtps2pd xmm4, xmm0
0x0cf2ae  test     r15b, r15b
0x0cf2b1  jne      0xcf2d4                    ; 标志非零 -> 跳过缩放
0x0cf2b3  mulss    xmm1, [0x349920]           ; f32 0.25 -> T_B
0x0cf2bb  cvtps2pd xmm4, xmm1                 ; 覆盖上面的加倍结果
0x0cf2be  mulss    xmm2, [0x34995c]           ; f32 5.0  -> T_A
0x0cf2c9  mulss    xmm3, [0x34995c]           ; f32 5.0  -> T_C
0x0cf2d4  cmp      byte ptr [r14 + 0x135], 0
0x0cf2fb  call     0x1800d4880                ; 裁剪调用
0x0cf300  cmp      byte ptr [r14 + 0x1c5], 0  ; 冗余裁剪开关
```

三处更正：

1. **训练前半段加倍的是 `0x128`，不是首轮所说的"阈值 A"**。判据是 `当前步 < 总迭代数/2`。
2. **系数与偏移的对应是 `0x128 → ×0.25`、`0x124 → ×5.0`、`0x130 → ×5.0`。** 首轮按偏移升序书写，把系数错位了一格；照其实现会把 0.25 用到错误的阈值上。
3. **两种调整互斥，不叠加。** 缩放分支里的 `cvtps2pd xmm4, xmm1`（`0xcf2bb`）会覆盖 `0xcf2ab` 写入的加倍值，因此前半段加倍只在 `r15b != 0`（跳过缩放）时才生效。
