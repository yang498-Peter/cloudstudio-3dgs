...
| K2 | C1 + cull 每 100 步、仅限 14000–15000 窗口（新旋钮 `post_refine_cull_until`） | 排队（A1 之后，插在 L1/R1 前） | | | | 假设：清掉 reset 遗留的死质量后留 5k 步重新优化，而不是持续侵蚀 |
| A1 | C1 + `da2_depth_weight` 0.15 → 0 | 排队（K1 之后） | | | | 单目深度平滑先验是未单独测过的糊化嫌疑（竞品不用 DA2） |
