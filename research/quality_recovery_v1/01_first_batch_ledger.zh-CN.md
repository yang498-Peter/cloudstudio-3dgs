...
| L1 | C1 + `lidar_alpha_weight` 0.1 → 0.05 | 11.49M | 0.287（−3.7%） | 0.536（−5%） | 0.427 mm / 11.5 | **中性偏负**：alpha 权重不是杠杆，保留 0.1 |
| R1 | C1 + `lidar_range_weight` 0.05 → 0 | 11.46M | **0.330（+11%，参考的 83%）** | 0.553（−2%，噪声内） | 0.430 mm / 11.6 | **赢（第二个赢臂）**：砾石逐颗、门的窗格出现；离轨迹锐度持平、无新增漂浮；离轨迹 PSNR vs 参考 −0.5 dB 记为待观察。LiDAR 距离项确实在压平细纹理（监督冲突①得到第一个正向证据）。**以 R1 配方启动四切片交付（R1d）** |

**R1d 交付 Tile_1（室内）读数（2026-09-07 19:02）**：训练视角锐度 0.105（G9d tile1 0.088，+19%；参考同帧 0.562，我们只有参考的 19%），离轨迹 0.148（G9d 0.201，−26%），人口 3.34M（cap 15M 从未绑定）。结论：室内是差距的主体——(a) 室内照片暗、纹理弱，投影梯度小，5e-5 阈值下自然出生少，密度杠杆在室内是阈值不是 cap；(b) 去掉 LiDAR 距离项在室外提锐度，在室内损几何（离轨迹 −26%），即最优监督权重按区域不同。下一步室内专项臂（在 Tile_1 上）：tile1_C1d（距离项保留）与 tile1_T1a（R1 配方 + 阈值 3.5e-5），排在 P2 之后；长期方向是按局部梯度统计自适应阈值/权重而不是按切片配置。

**R1d Tile_2 读数（21:42）**：训练视角 0.195（G9d tile2 0.240，**−19%**），离轨迹 0.413（G9d 0.361，+14%），PSNR vs 参考 −0.8 dB。加上 Tile_1 的 +19%/−26%，R1 配方（去 LiDAR 距离项）在三个切片上方向各不相同，只有 Tile_0 是双正。**方法论修正：单个 Tile_0 臂的结论不能外推到全场景；此后判臂至少要在一个室外切片 + 一个室内切片上重复，两者都不劣于噪声带才算赢。** 全场景 R1d 对 G9 交付的同帧对比（battery / matched strips）作为这次交付的最终裁决。

**事故（2026-09-07 19:21 → 09-10）**：室内臂的等待脚本匹配到了 P2 首次失败留下的 "exit" 行，在交付链还在训练 tile2/tile3 时提前启动了 tile1_C1d / tile1_T1a，两个训练器同时占 GPU（tile3 单独就要 15 GiB）；tile3 爬到 step 3770 后，09-10 约 18:00 宿主重启把所有进程杀光。交付未合并，损失约 3.5 天 GPU 时间。已修正等待脚本（只认本次等待开始之后新增的 "exit 0"，且检测到 python 训练器在跑时不启动）。可用的结果：tile1_R1d、tile2_R1d 完成；**tile1_C1d（室内，C1 配方保留距离项）**：训练视角 0.086（G9d 0.088、R1d 0.105）、离轨迹 0.192（G9d 0.201、R1d 0.148）——室内三种配方都在 G9d 附近，这些旋钮在室内不起作用，室内需要别的东西（更多有效出生：阈值 / 初始化密度 / 曝光）。

E1 首次启动 10 s 失败：exact 生命周期要求 `refine_scale2d_stop_iter == mcmc_refine_stop_iter`（顶层键），已同步改为 21000；P2 首次启动 8 s 失败：`lidar_alpha_dilation_radius_px` 要求 alpha 权重 >0，已置 0。两者重新排在交付之后（B1 → E1 → P2）。
