...
- [ ] Gate 1 完整 MCMC：已加入 clean-lock/build-feature/native-op fail-closed 审计、实际 position-noise 探针、refine/relocate/add/opacity/scale 遥测、NaN/Inf 守卫、连续/恢复 checkpoint 全状态比较及签名 Exit Gate 验证器。澳洲 RTX 5070 Ti 已报告非零噪声和 `24→29` add，但尚未按统一契约提交 fused perturb、强制 relocation 和 resume 等价证据；本机旧扩展仍为 `FAIL`，总门保持未关闭
