...
# "appearance": {
#   "mode": "sh", "maximum_degree": 3, "degree_interval": 1000,
#   "rest_lr_scale": 0.05
# }
...
- [ ] Gate 2 训练器质量地基：KNN 每点尺度 + 米制 means LR/MCMC noise 联动的源码、CPU 契约和真实 376,906 点双重放已通过；定标 SHA `fcce9850...73d4`，中位尺度 `0.097916 m`、effective means LR `3.133e-4`、effective noise LR `8148.6`。11×11 local masked SSIM、robust log-range Huber、SH0/SHN 与 degree 0→3 确定性调度的源码/CPU/checkpoint 契约已通过；真实 GPU 短 A/B、正则、means LR decay、周期 golden eval/best checkpoint 仍为 `NOT_RUN`
