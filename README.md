...
python tools/build_trainer_ab_matrix.py `
  --base-config configs/trainer_gate2_ab_base.example.json `
  --experiment-id gs2-factor4-gate2 `
  --output G:\3dgs-runs\gs2-factor4-gate2
...
python tools/summarize_trainer_ab.py `
  --matrix G:\3dgs-runs\gs2-factor4-gate2\ab_matrix_manifest.json `
  --output G:\3dgs-runs\gs2-factor4-gate2\ab_report.json
...
# 澳洲 30k 路线：保留平台期 checkpoint，避免 only latest+best 丢失中间证据。
# 正式 Trainer JSON 使用 checkpoint_keep_every（例如 2000）；默认 0 保持旧契约。
pwsh -File train\run_gate2_synthetic_acceptance_local.ps1 `
  -Output G:\3dgs-runs\full_mcmc_checkpoint_retention `
  -CheckpointKeepEvery 20
...
- [ ] Gate 2 训练器质量地基：以澳洲 `machine-b/uk-quality` 为优先路线，P10 8k control 为 PSNR `16.45`，seed-43 30k 的 `best_golden@7000` 刷新为 PSNR `17.07`、p10 `16.73`、SSIM `0.5622`；hard/soft exposure anchor 与 error-weighted v1 均已受控拒绝。GS2 同签名 124 图正式证据已完成 legacy reference、澳洲 P5 与 KNN-only：P5 外观显著领先但 depth 回归，KNN-only 相对 legacy 为 `+0.259 dB`、LPIPS 改善 `0.00536`，depth 恶化 `0.08897 m`，结论仍是 MIXED。旧 SH-only 在同步澳洲新路线后于 5000 步停止且无最终 Manifest，local-SSIM 未完成，不能冒名关闭五臂 Gate。当前新增 `checkpoint_keep_every` 修复澳洲 30k only-latest+best 丢失平台点的问题；下一增量按澳洲结论推进 α·T 真聚合的 error-weighted v2，而不是回到已否决的锚定/旧参数搜索。
