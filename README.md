...
- [ ] Gate 2 训练器质量地基：KNN 每点尺度 + 米制 means LR/MCMC noise 联动的源码、CPU 契约和真实 376,906 点双重放已通过；定标 SHA `fcce9850...73d4`，中位尺度 `0.097916 m`、effective means LR `3.133e-4`、effective noise LR `8148.6`。真实 GPU 短 A/B、SH、local masked SSIM、robust log-range、正则、周期 golden eval/best checkpoint 仍为 `NOT_RUN`
