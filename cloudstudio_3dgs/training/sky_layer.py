...
def load_prebaked_sky_layer(path: Path) -> dict[str, np.ndarray]:
    """Load a photo-baked far-field dome saved as checkpoint-convention params.

    The file holds log-scales and logit-opacities exactly as a checkpoint
    would, so the arrays concatenate onto a trained model without any domain
    conversion. Only the SH0 band is taken; higher bands are zeroed against
    the destination model's band count by the caller.
    """
    import torch
